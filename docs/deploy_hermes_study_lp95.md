...
- `POST /api/evaluate`：先保存试卷任务，再异步执行 OCR 和 AI 分析，返回任务记录。
- `GET /api/evaluations/:id`：查看任务状态、逐页 OCR 进度和最终结论。
- `GET /api/evaluations/:id/files/:index`：复看已保存的原始附件。
- `POST /api/evaluations/:id/retry`：对失败任务重新分析。
...
- 上传附件保存在 `uploads/`，记录中保存页码、正面/反面/附页顺序；应用重启后会继续处理未完成任务。
